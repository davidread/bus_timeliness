            ...
            if line.text == route_name:
                ...
                for journey_pattern in service.findall('.//txc:JourneyPattern', ns):
                    ...
                    if direction_elem is not None:
                        ...
                        if direction == "inbound" and ("inbound" in direction_text.lower() or "oxford" in direction_text.lower()):
                            direction_match = True
                        elif direction == "outbound" and ("outbound" in direction_text.lower() or "london" in direction_text.lower()):
                            direction_match = True
                        ...
                        if direction_match:
                            ...
                            for jp_section_ref in journey_pattern.findall('.//txc:JourneyPatternSectionRef', ns):
                                section_id = jp_section_ref.text
                                
                                # Find the corresponding JourneyPatternSection
                                for jp_section in root.findall(f'.//txc:JourneyPatternSection[@id="{section_id}"]', ns):
                                    ...
                                    for timing_link in jp_section.findall('.//txc:JourneyPatternTimingLink', ns):
                                        ...
                                        if from_stop is not None:
                                            ...
                                            if stop_id in stops_dict:
                                                ...
                                                if {"name": stop_name, "atco_code": stop_id} not in stop_sequence:
                                                    stop_sequence.append({"name": stop_name, "atco_code": stop_id})
